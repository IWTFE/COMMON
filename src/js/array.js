var Array_={
  ...
  * @see every
  */
   some : function(source, iterator, thisObject) {
   var i = 0,
       len = source.length;
   for (; i < len; i++) {
 ...
 * @see reduce
 */
reduce : function(source, iterator, initializer) {
     var i = 0,
         l = source.length,
         found = 0;
 ...
 * @see reduce
 */
map : function(source, iterator, thisObject) {
    var results = [],
        i = 0,
        l = source.length;
    for (; i < l; i++) {
 ...
 */
 lastIndexOf : function (source, match, fromIndex) {
    var len = source.length;
 ...
 * @name every
 * @function
 * @grammar every(source, iterator[,thisObject])
 * @param {Array} source 需要判断的数组.
 * @param {Function} iterator 判断函数.
 * @param {Object} [thisObject] 函数调用时的this指针，如果没有此参数，默认是当前遍历的数组
 * @return {boolean} 判断结果.
 * @see some
 */
  every : function(source, iterator, thisObject) {
    var i = 0,
        len = source.length;
    for (; i < len; i++) {
        if (i in source && !iterator.call(thisObject || source, source[i], i)) {
 ...
 */
  ...
  module.exports= {
      unique: Array_.unique,
      some:Array_.some,
      removeAt:Array_.removeAt,
      remove:Array_.remove,
      reduce:Array_.reduce,
      lastIndexOf:Array_.lastIndexOf,
      indexOf:Array_.indexOf,
      hash:Array_.hash,
      find:Array_.find,
      filter:Array_.filter,
      every:Array_.every,
      each:Array_.each,
      contains:Array_.contains
